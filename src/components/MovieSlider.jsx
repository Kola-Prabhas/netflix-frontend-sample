...
const MovieSlider = ({ category }) => {
	...
	return (
		...
		>
			...
			<div
				className="flex space-x-8 py-4 overflow-x-scroll scrollbar-hide transition-all ease duration-400"
				ref={sliderRef}
			>
				{content.map((item) => (
					<Link
						to={`/watch/${item.id}`}
						className="min-w-[280px] relative hover:scale-110 transition-all ease-in-out duration-300 hover:z-10 top-0 left-0"
						key={item.id}
					>
